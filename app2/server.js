...
try {
    ...
    console.log('App2: Data loaded.');
} catch (err) {
    inMemoryData = [];
    console.log('App2: Data started empty.');
}
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'PUT' && req.url.startsWith('/api/formulas/')) {
    ...
    if (req.method === 'DELETE' && req.url.startsWith('/api/formulas/')) {
        const id = parseInt(req.url.split('/').pop());
...
server.listen(PORT, () => {
    console.log(`App2 Server running at http://localhost:${PORT}`);
});